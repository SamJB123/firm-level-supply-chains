...
def parse_document(document: SourceDocument) -> list[ParsedEvidence]:
    ...
    if document.document_type == "annual_report":
        if any(keyword in normalized_text.lower() for keyword in SUPPLY_CHAIN_KEYWORDS):
            evidence.append(
                ParsedEvidence(
                    evidence_id=_make_evidence_id(document.document_id, "annual-report-undisclosed-supplier"),
                    country=Country.AUSTRALIA,
                    relation_type=RelationType.UNDISCLOSED_SUPPLIER,
                    reporter_name=document.company_name,
                    counterparty_name=f"Undisclosed Supplier · {document.company_name} · {document.document_id}",
                    ...
                    parser_method="australia_annual_report_placeholder",
